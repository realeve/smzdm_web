        ...
        initCanvas2() {
            function i() {
                if (ctx.clearRect(0, 0, w, h), firstTime)
                    for (q = [{ x: 0, y: .7 * h + bandwidth, v_x: 0, v_y: 0 }, { x: 0, y: .7 * h - bandwidth, v_x: 0, v_y: 0 }], verticals.push(q[0]), verticals.push(q[1]); q[1].x < w + bandwidth;) firstDraw(q[0], q[1]);
                else
            ...
            function firstDraw(t, e) {
                ...
                //ctx.stroke()
                ctx.fill;
                q[0] = q[1], q[1] = { x: i, y: a, v_x: 0, v_y: 0 }, verticals.push(q[1]), fillStyles.push(ctx.strokeStyle), verticals_side.push(q[1])
            }
